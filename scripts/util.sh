...
#   echo
  ...
  # if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    #             set -x
  #   peer chaincode invoke -o orderer.ptunstad.no:7050 -C $CHANNEL_NAME -n myccds -c '{"Args":["set","c","77"]}' >&log.txt  ## '{"Args":["invoke","a","b","10"]}'
  #   res=$?
    #             set +x
  # else
    #             set -x
  #   peer chaincode invoke -o orderer.ptunstad.no:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n myccds -c '{"Args":["set","c","77"]}' >&log.txt
  #   res=$?
    #             set +x
  # fi
  # cat log.txt
